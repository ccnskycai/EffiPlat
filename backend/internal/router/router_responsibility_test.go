...
func TestResponsibilityRoutes(t *testing.T) {
	...
	var createdTestResp model.Responsibility

	t.Run("POST /responsibilities - Create Responsibility", func(t *testing.T) {
		newResp := model.Responsibility{
	...
	t.Run("GET /responsibilities - List Responsibilities", func(t *testing.T) {
		...
		router.CreateTestResponsibility(t, app.Router, authToken, &model.Responsibility{Name: "Another Test Resp", Description: "Desc 2"})
		...
		var listResp struct {
			Data utils.PaginatedResponse `json:"data"`
		}
	...
	t.Run("PUT /responsibilities/:id - Update Responsibility", func(t *testing.T) {
		updatePayload := model.Responsibility{
