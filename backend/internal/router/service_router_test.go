...
func TestServiceAndTypeManagementRoutes(t *testing.T) {
	...
	t.Run("Setup_CreateTestServiceType", func(t *testing.T) {
		...
		createSTReq := models.CreateServiceTypeRequest{
			Name:        stName,
		...
		var stRespBody struct {
			Data models.ServiceType `json:"data"`
		}
	...
	t.Run("UpdateServiceType_Success", func(t *testing.T) {
		...
		updateReq := models.UpdateServiceTypeRequest{
			Name:        &updatedName,
	...
	t.Run("UpdateServiceType_Fail_NotFound", func(t *testing.T) {
		...
		updateReq := models.UpdateServiceTypeRequest{
			Name:        models.StringPtr("NonExistentName"),
			Description: &updatedDesc,
		}
		payloadBytes, _ := json.Marshal(updateReq)
	...
	t.Run("UpdateServiceType_Fail_DuplicateName", func(t *testing.T) {
		...
		createAuxSTReq := models.CreateServiceTypeRequest{
			Name:        auxName,
		...
		updateReq := models.UpdateServiceTypeRequest{
			Name: models.StringPtr(auxName), // Attempt to use the name of the auxiliary ST
		}
		payloadBytes, _ := json.Marshal(updateReq)
		updateURL := fmt.Sprintf("/api/v1/service-types/%d", createdServiceTypeID)
		req, _ := http.NewRequest("PUT", updateURL, bytes.NewBuffer(payloadBytes))
	...
	t.Run("ListServiceTypes_Success_WithDataAndPagination", func(t *testing.T) {
		...
		for _, name := range stNamesToCreate {
			createReq := models.CreateServiceTypeRequest{Name: name, Description: "Paging test ST"}
			...
			var resp struct{ Data models.ServiceType `json:"data"` }
		...
		var listResp struct {
			Data struct {
				Items    []models.ServiceType `json:"items"`
	...
	t.Run("DeleteServiceType_Success", func(t *testing.T) {
		...
		createTempSTReq := models.CreateServiceTypeRequest{Name: tempSTName}
		...
		var respCreate struct{ Data models.ServiceType `json:"data"` }
...
t.Run("CreateService_Success", func(t *testing.T) {
	...
	t.Run("CreateService_Fail_DuplicateName", func(t *testing.T) {
		...
		firstCreateReq := models.CreateServiceRequest{
		...
		var resp1Body struct { Data models.Service `json:"data"` }
		...
		duplicateCreateReq := models.CreateServiceRequest{
	...
	t.Run("ListServices_Success_WithDataAndPagination", func(t *testing.T) {
		...
		for _, name := range auxServiceNames {
			...
			var resp struct{ Data models.Service `json:"data"` }
			err := json.Unmarshal(w.Body.Bytes(), &resp)
			require.NoError(t, err)
	...
	t.Run("UpdateService_Success", func(t *testing.T) {
		...
		createOtherSTReq := models.CreateServiceTypeRequest{Name: otherSTName}
		...
		var stRespCreate struct{ Data models.ServiceType `json:"data"` }
		...
		updatedStatus := models.ServiceStatusInactive
		...
		updateReq := models.UpdateServiceRequest{
			...
			Description:   models.StringPtr(updatedDesc),
			Version:       models.StringPtr(updatedVersion),
			Status:        &updatedStatus,
			ExternalLink:  models.StringPtr(updatedLink),
	...
	t.Run("UpdateService_Fail_NotFound", func(t *testing.T) {
		...
		updateReq := models.UpdateServiceRequest{Description: models.StringPtr("Attempt to update non-existent")}
		payloadBytes, _ := json.Marshal(updateReq)
	...
	t.Run("UpdateService_Fail_InvalidPayload", func(t *testing.T) {
		...
		invalidStatusValue := models.ServiceStatus("this-is-not-a-valid-status") // Cast to models.ServiceStatus
		updateReq := models.UpdateServiceRequest{Status: &invalidStatusValue}
		payloadBytes, _ := json.Marshal(updateReq)
		updateURL := fmt.Sprintf("/api/v1/services/%d", testServiceID)
		req, _ := http.NewRequest("PUT", updateURL, bytes.NewBuffer(payloadBytes))
		req.Header.Set("Content-Type", "application/json")
	...
	t.Run("UpdateService_Fail_DuplicateName", func(t *testing.T) {
		...
		createAuxReq := models.CreateServiceRequest{
			Name:          auxServiceName,
			ServiceTypeID: createdServiceTypeID,
			Status:        models.ServiceStatusActive,
		}
		...
		var auxRespBody struct{ Data models.Service `json:"data"` }
		err := json.Unmarshal(auxW.Body.Bytes(), &auxRespBody)
		require.NoError(t, err)
		...
		updateReq := models.UpdateServiceRequest{Name: models.StringPtr(auxServiceName)}
	...
	t.Run("UpdateService_Fail_ServiceTypeNotFound", func(t *testing.T) {
		...
		updateReq := models.UpdateServiceRequest{ServiceTypeID: &nonExistentSTID}
		payloadBytes, _ := json.Marshal(updateReq)
		updateURL := fmt.Sprintf("/api/v1/services/%d", testServiceID)
		req, _ := http.NewRequest("PUT", updateURL, bytes.NewBuffer(payloadBytes))
		req.Header.Set("Content-Type", "application/json")
