...
func (r *gormServiceRepository) GetByID(ctx context.Context, id uint) (*model.Service, error) {
	var service model.Service
	err := r.db.WithContext(ctx).Preload("ServiceType").First(&service, id).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, model.ErrServiceNotFound // Assuming ErrServiceNotFound is defined in models
		}
		return nil, err
	}
	return &service, nil
...
func (r *gormServiceRepository) GetByName(ctx context.Context, name string) (*model.Service, error) {
	var service model.Service
...
func (r *gormServiceRepository) List(ctx context.Context, params model.ServiceListParams) ([]model.Service, int64, error) {
	var services []model.Service
	var totalCount int64

	query := r.db.WithContext(ctx).Model(&model.Service{}).Preload("ServiceType")
...
func (r *gormServiceRepository) Update(ctx context.Context, service *model.Service) error {
...
func (r *gormServiceRepository) CountServicesByServiceTypeID(ctx context.Context, serviceTypeID uint) (int64, error) {
	var count int64
	err := r.db.WithContext(ctx).Model(&model.Service{}).Where("service_type_id = ?", serviceTypeID).Count(&count).Error
	return count, err
}
