...
func setupServiceTestDBAndRepo(t *testing.T) (*gorm.DB, ServiceRepository, sqlmock.Sqlmock, func()) {
	...
	// Migrate the schema
	err = gormDB.AutoMigrate(&model.ServiceType{}, &model.Service{})
	require.NoError(t, err)
...
func createTestServiceType(t *testing.T, db *gorm.DB, name string) *model.ServiceType {
	st := &model.ServiceType{Name: name, Description: "Test Service Type"}
...
func TestGormServiceRepository_List(t *testing.T) {
	...
	servicesData := []model.Service{
		{Name: "Service Alpha", ServiceTypeID: st1.ID, Status: model.ServiceStatusActive},
		{Name: "Service Beta", ServiceTypeID: st1.ID, Status: model.ServiceStatusInactive},
		{Name: "Service Gamma", ServiceTypeID: st2.ID, Status: model.ServiceStatusActive},
		{Name: "Another Alpha", ServiceTypeID: st2.ID, Status: model.ServiceStatusDevelopment},
	}
	...
	t.Run("list all no filters", func(t *testing.T) {
		params := model.ServiceListParams{Page: 1, PageSize: 10}
	...
	t.Run("list with name filter", func(t *testing.T) {
		params := model.ServiceListParams{Page: 1, PageSize: 10, Name: "Alpha"}
		services, total, err := repo.List(ctx, params)
		assert.NoError(t, err)
	...
	t.Run("list with status filter", func(t *testing.T) {
		params := model.ServiceListParams{Page: 1, PageSize: 10, Status: string(model.ServiceStatusActive)}
		services, total, err := repo.List(ctx, params)
		assert.NoError(t, err)
		assert.Equal(t, int64(2), total)
		assert.Len(t, services, 2)
		for _, s := range services {
			assert.Equal(t, model.ServiceStatusActive, s.Status)
		}
	})

	t.Run("list with service type id filter", func(t *testing.T) {
		params := model.ServiceListParams{Page: 1, PageSize: 10, ServiceTypeID: st1.ID}
		services, total, err := repo.List(ctx, params)
		assert.NoError(t, err)
		assert.Equal(t, int64(2), total)
		assert.Len(t, services, 2)
	...
	t.Run("list empty result", func(t *testing.T) {
		params := model.ServiceListParams{Page: 1, PageSize: 10, Name: "NonExistentNameForList"}
		services, total, err := repo.List(ctx, params)
		assert.NoError(t, err)
...
func TestGormServiceRepository_Update(t *testing.T) {
	...
	updatedVersion := "2.0"
	updatedStatus := model.ServiceStatusActive
...
func TestGormServiceRepository_Delete(t *testing.T) {
	...
	var softDeletedService model.Service
...
func TestGormServiceRepository_CountServicesByServiceTypeID(t *testing.T) {
	...
	// Services for st1
	_ = db.Create(&model.Service{Name: "S1T1", ServiceTypeID: st1.ID, Status: model.ServiceStatusActive})
	_ = db.Create(&model.Service{Name: "S2T1", ServiceTypeID: st1.ID, Status: model.ServiceStatusActive})

	// Service for st2
	_ = db.Create(&model.Service{Name: "S1T2", ServiceTypeID: st2.ID, Status: model.ServiceStatusActive})
