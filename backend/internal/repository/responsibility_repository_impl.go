...
func (r *gormResponsibilityRepository) List(ctx context.Context, params model.ResponsibilityListParams) ([]model.Responsibility, int64, error) {
	r.logger.Debug("GORM: Listing responsibilities", zap.Any("params", params))
	var responsibilities []model.Responsibility
	var total int64

	tx := r.db.WithContext(ctx).Model(&model.Responsibility{})
...
func (r *gormResponsibilityRepository) GetByID(ctx context.Context, id uint) (*model.Responsibility, error) {
	r.logger.Debug("GORM: Getting responsibility by ID", zap.Uint("id", id))
	var resp model.Responsibility
...
func (r *gormResponsibilityRepository) Update(ctx context.Context, resp *model.Responsibility) (*model.Responsibility, error) {
	...
	// Consider using .Model(&model.Responsibility{}).Where("id = ?", resp.ID).Updates(resp) for partial updates
...
func (r *gormResponsibilityRepository) Delete(ctx context.Context, id uint) error {
	...
	result := r.db.WithContext(ctx).Delete(&model.Responsibility{}, id)
	if result.Error != nil {
