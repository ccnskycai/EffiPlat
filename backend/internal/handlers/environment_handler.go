...
// @Accept json
// @Produce json
// @Param environment body models.CreateEnvironmentRequest true "Environment creation request"
// @Success 201 {object} response.SuccessResponse{data=models.EnvironmentResponse} "Environment created successfully"
// @Failure 400 {object} response.ErrorResponse "Invalid request payload or validation error"
// @Failure 409 {object} response.ErrorResponse "Environment with the same slug already exists"
...
func (h *EnvironmentHandler) CreateEnvironment(c *gin.Context) {
	...
	if req.Name == "" || req.Slug == "" {
		response.Error(c, http.StatusBadRequest, "Name and Slug are required")
		return
	}
...
// @Success 200 {object} response.SuccessResponse{data=response.PaginatedData{items=[]models.EnvironmentResponse}} "Environments retrieved successfully"
// @Failure 500 {object} response.ErrorResponse "Internal server error"
...
func (h *EnvironmentHandler) GetEnvironments(c *gin.Context) {
	...
	listParams := models.EnvironmentListParams{
		Page:     page,
		PageSize: pageSize,
	}
	...
	if err != nil {
		...
		response.Error(c, http.StatusInternalServerError, "Failed to retrieve environments: "+err.Error())
		return
	}

	response.Paginated(c, environments, total, page, pageSize)
}
...
// @Success 200 {object} response.SuccessResponse{data=models.EnvironmentResponse} "Environment retrieved successfully"
// @Failure 400 {object} response.ErrorResponse "Invalid environment ID format"
...
func (h *EnvironmentHandler) GetEnvironmentByID(c *gin.Context) {
	...
	if err != nil {
		response.Error(c, http.StatusBadRequest, "Invalid environment ID format")
		return
	}
	...
	if err != nil {
		if errors.Is(err, utils.ErrNotFound) {
			...
			response.Error(c, http.StatusInternalServerError, "Failed to retrieve environment: "+err.Error())
		}
		return
	}
	response.OK(c, env)
...
func (h *EnvironmentHandler) GetEnvironmentBySlug(c *gin.Context) {
	slug := c.Param("slug")
	if slug == "" {
		response.Error(c, http.StatusBadRequest, "Slug parameter is required")
		return
	}
...
// @Produce json
// @Param id path string true "Environment ID"
// @Param environment body models.UpdateEnvironmentRequest true "Environment update request"
// @Success 200 {object} response.SuccessResponse{data=models.EnvironmentResponse} "Environment updated successfully"
// @Failure 400 {object} response.ErrorResponse "Invalid environment ID format or request payload"
// @Failure 404 {object} response.ErrorResponse "Environment not found"
// @Failure 409 {object} response.ErrorResponse "Environment with the same slug already exists (if slug is changed)"
// @Failure 500 {object} response.ErrorResponse "Internal server error"
